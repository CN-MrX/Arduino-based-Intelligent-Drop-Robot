...
def getAngle():
    ...
    k=0
    angleMatrix=[0 for i in range(19)]
    ...
    for angle in range(0,91,5):
        ...
        M_1 = cv.getRotationMatrix2D(center, angle, 1)
        rotated_1 = cv.warpAffine(template, M_1, (w, h))
        cv.imwrite("E:/Made By ZYX/Learning/University/TH-2/Robot//template2.jpg", rotated_1)

        # Apply template Matching
        res = cv.matchTemplate(img,rotated_1,eval('cv.TM_CCOEFF'))

        minNumber = abs(res[0][0])
        for i in range(len(res)):
            for j in range(len(res[i])):
                if(abs(res[i][j])<minNumber):
                    minNumber = abs(res[i][j])

        angleMatrix[k]=minNumber
        print(angleMatrix[k])
        k=k+1
    ...
    print("数组：",angleMatrix)

    tempIndex=0
    finalAngle=0
    tempValue = angleMatrix[0]

    for index in range(len(angleMatrix)):
        if(angleMatrix[index]<tempValue):
            tempValue=angleMatrix[index]
            tempIndex=index
            print(tempIndex)

    finalAngle = tempIndex*5
    return finalAngle
